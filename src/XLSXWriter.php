...
class XLSXWriter
{
    ...
    protected function initializeSheet($sheet_name)
    {
        ...
        $this->sheets[$sheet_name] = (object) array(
            ...
            'row_count' => 0,
            'file_writer' => new XLSXWriterBufferWriter($sheet_filename),
            'columns' => array(),
            'merge_cells' => array(),
    ...
    {
        $number_format_idx = self::addToListGetIndex($this->number_formats, $number_format);
        $lookup_string = $number_format_idx . ";" . $cell_style_string;
        $cell_style_idx = self::addToListGetIndex($this->cell_styles, $lookup_string);
    ...
    protected function writeCell(XLSXWriterBufferWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
    {
    ...
    protected function styleFontIndexes()
    {
        ...
        foreach ($this->cell_styles as $i => $cell_style_string) {
            ...
            if (isset($style['border']) && is_string($style['border'])) {
                ...
                $style_indexes[$i]['border_idx'] = self::addToListGetIndex($borders, implode(",", $border_value));
            }
    ...
    protected function writeStylesXML()
    {
        ...
        $file = new XLSXWriterBufferWriter($temporary_filename);
    ...
    public static function xmlspecialchars($val)
    {
        ...
        static $goodchars = "                                 ";
        return strtr(htmlspecialchars($val, ENT_QUOTES | ENT_XML1, '', false), $badchars, $goodchars); //strtr appears to be faster than str_replace
    }
    //------------------------------------------------------------------
